...
_slotState._pairsIter = function(table)
...
setmetatable(_slotState.states, {
	__newindex = function(t, k, v)
		...
		if type(k) == "number" and k == math.floor(k) then
			...
			assert(k == "auto", string.format("States can't have a non-integer index ([%s] => %s)", type(k), k))
...
function StateManager.destroy(id)
	for iSet, state in _slotState._pairsIter(_slotState.states) do
		if state._id == id then
